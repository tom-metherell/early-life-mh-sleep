...
age_46 %<>% select(
  ...
  BD10ENFA, # Energy/fatigue score
  BD10MAL, # Total Malaise score
  BD10MBMI, # Body mass index (nurse-measured)
...
for(dataset in c("age_5d", "age_5e", "age_5f", "age_10", "age_10dv", "age_16", "age_16arith", "age_16dv", "age_16school", "age_26", "age_30", "age_42", "age_46", "CLOSER_env", "CLOSER_BMI", "CLOSER_SES", "response")){
  data_clean <- full_join(data_clean, get(dataset), by = "bcsid")
}
```

## Deriving exposure variables
To make the data useful, we produce a number of derived variables which are more informative than the raw source data. This applies to both the exposures...
...
```{r control_variables}
# Storing paternal social class at birth as a separate variable
data_clean %<>% mutate(dv_sc_birth = a0014)
...
# Deriving maximum parental education at age 5 and removing mother variable
data_clean %<>% rowwise() %>% mutate(dv_par_edu_5 = max(e191, e192))
data_clean %<>% select(-e191)
...
# Recoding "seen Dr/specialist for mental health problems" into a single binary variable and removing individual variables
data_clean %<>% mutate(dv_mhprb = case_when(
  if_any(B10MHPRB1:B10MHPRB6, ~. == 1) ~ 1,
  if_all(B10MHPRB1:B10MHPRB6, ~. == 2) ~ 0
))
data_clean %<>% select(-(B10MHPRB1:B10MHPRB6))

# Deriving abnormal sleep and removing raw variable
data_clean %<>% mutate(
  dv_sr_sleep_abn = case_when(
    B10HSLEEP < 6 | B10HSLEEP > 9 ~ 1,
    B10HSLEEP >= 6 & B10HSLEEP <= 9 ~ 0
  ),
  dv_sr_sleep_short = case_when(
    B10HSLEEP < 6 ~ 1,
    B10HSLEEP >= 6 ~ 0
  ),
  dv_sr_sleep_long = case_when(
    B10HSLEEP > 9 ~ 1,
    B10HSLEEP <= 9 ~ 0
  )
)
data_clean %<>% select(-B10HSLEEP)

# Recoding labelled variables as factors, taking the values of the data labels
data_clean[c("a0014", "a0043b", "a0255", "e216a", "c3_11", "BDSTYPE", "B9TEN", "ameni", "brfed", "crowd", "resmove", "tenure", "fclrg90", "dv_sc_birth", "dv_sc_age_5", "dv_sc_age_16", "dv_incsource", "dv_marital")] %<>% as_factor(., only_labelled = FALSE)
